  ...
  class GDapiSP {
      ...
      return fetch("https://gdbrowser.com/u/" + args.user)
        ...
        .then((data) => {
          switch (args.menu.toLowerCase()) {
            case "stars":
              return data.match(/<span id="stars">(\d+)/)?.[1] || "";
            case "moons":
              return data.match(/<span id="moons">(\d+)/)?.[1] || "";
            case "diamonds":
              return data.match(/<span id="diamonds">(\d+)/)?.[1] || "";
            case "creator points":
              return data.match(/<span id="cp">(\d+)/)?.[1] || "";
            case "secret coins":
              return data.match(/<span id="coins">(\d+)/)?.[1] || "";
            case "user coins":
              return data.match(/<span id="usercoins">(\d+)/)?.[1] || "";
            case "demons":
              return data.match(/<span id="demons">(\d+)/)?.[1] || "";
            case "global rank":
    ...
    fetchIconFromUser(args) {
      ...
      return fetch("https://gdbrowser.com/u/" + args.user)
        .then((response) => response.text())
        .then((data) => {
          if (gamemode === "death effect") {
            return data.match(/title="Death Effect (\d+)".*?>/)?.[1] || "";
          } else {
            return (data.match(new RegExp(`gdicon iconID=(\\d+) iconForm="${gamemode}"`)) || [])[1] || "";
          }
        })
        .catch((error) => {
          console.error(error);
      ...
      return fetch("https://gdbrowser.com/u/" + args.user)
        ...
        .then((data) => {
          switch (args.color) {
            ...
            case "glow color":
            case "enabled glow":
              return data.match(/<gdicon.*?iconForm="icon".*?colG="(\d+)".*?>/)?.[1] || "";
          }
        })
        .catch((error) => {
          console.error(error);
