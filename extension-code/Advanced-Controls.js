...
// ID: SPadvControl
// Description: Advanced Control Blocks
// By: SharkPool
...
(function (Scratch) {
  "use strict";
  if (!Scratch.extensions.unsandboxed) throw new Error("Advanced Control must run unsandboxed");
  ...
  class SPadvControl {
    ...
    getPrivateVars() {
      const vars = typeof Blockly === "undefined" ? [] : Blockly.getMainWorkspace()
        ...
        .filter((model) => model.isLocal).map((model) => (model.name));
      ...
      for (const target of runtime.targets) {
        ...
        const blockIDs = Object.keys(blocks._blocks)
          .filter(key => {
            ...
            return block.opcode === "SPadvControl_onCall" && blocks.getBlock(block.inputs.CALL.block).fields.TEXT.value === callID;
          })
